...
juju relate server-mas oidc
```

To create the client, it's important to first retrive the MAS redirect URL. First we get the upstream OIDC provider ID (`<provider_id>`) generated by the charm:
```
...
Then we integrate with the exisitng synapse charm and create the backup
```
