...
def test_get_membership_room_id(harness: Harness, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    """
    harness.update_config({"enable_draupnir": True})
    harness.begin_with_initial_hooks()
    admin_access_token = token_hex(16)
    get_room_id = MagicMock()
    monkeypatch.setattr(synapse, "get_room_id", get_room_id)
...
@mock.patch("draupnir.Draupnir._admin_access_token", new_callable=PropertyMock)
def test_on_collect_status_blocked(
    ...
    monkeypatch.setattr(Draupnir, "get_membership_room_id", MagicMock(return_value=None))
    charm_state_mock = MagicMock()
    charm_state_mock.enable_draupnir = True
    harness.charm._draupnir._charm_state = charm_state_mock

    event_mock = MagicMock()
    harness.charm._draupnir._on_collect_status(event_mock)

    event_mock.add_status.assert_called_once_with(
        ops.BlockedStatus(
            "moderators not found and is required by Draupnir. Please, check the logs."
        )
    )
...
def test_on_collect_status_service_exists(
    ...
    enable_draupnir_mock = MagicMock()
    monkeypatch.setattr(Draupnir, "enable_draupnir", enable_draupnir_mock)

    event_mock = MagicMock()
    harness.charm._draupnir._on_collect_status(event_mock)

    event_mock.add_status.assert_not_called()
    enable_draupnir_mock.assert_not_called()
...
def test_on_collect_status_active(harness: Harness, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    harness.begin_with_initial_hooks()
    admin_access_token = token_hex(16)
    monkeypatch.setattr(Draupnir, "_admin_access_token", admin_access_token)
    membership_room_id_mock = MagicMock(return_value="123")
    monkeypatch.setattr(Draupnir, "get_membership_room_id", membership_room_id_mock)
    enable_draupnir_mock = MagicMock(return_value=None)
    monkeypatch.setattr(Draupnir, "enable_draupnir", enable_draupnir_mock)
    charm_state_mock = MagicMock()
    charm_state_mock.enable_draupnir = True
    harness.charm._draupnir._charm_state = charm_state_mock
    ...
    enable_draupnir_mock.assert_called_once()
...
def test_on_collect_status_api_error(harness: Harness, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    act: call _on_collect_status.
    assert: draupnir is not enabled.
    """
    harness.update_config({"enable_draupnir": True})
    harness.begin_with_initial_hooks()
    admin_access_token = token_hex(16)
...
def test_on_collect_status_admin_none(harness: Harness, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    arrange: start the Synapse charm, set server_name, mock container, mock _admin_access_token
        to be None.
    act: call _on_collect_status.
    assert: draupnir is not enabled and the model status is Maintenance.
    """
    harness.update_config({"enable_draupnir": True})
    harness.begin_with_initial_hooks()
    monkeypatch.setattr(Draupnir, "_admin_access_token", None)
    enable_draupnir_mock = MagicMock(return_value=None)
    monkeypatch.setattr(Draupnir, "enable_draupnir", enable_draupnir_mock)
    charm_state_mock = MagicMock()
    charm_state_mock.enable_draupnir = True
...
def test_enable_draupnir(harness: Harness, monkeypatch: pytest.MonkeyPatch) -> None:
    """
    arrange: start the Synapse charm, set server_name, mock calls to validate args.
    act: call enable_draupnir.
    ...
    admin_access_token = token_hex(16)
    monkeypatch.setattr(Draupnir, "_admin_access_token", admin_access_token)
    draupnir_user_mock = MagicMock()
    draupnir_access_token = token_hex(16)
    draupnir_user_mock.access_token = draupnir_access_token
    create_user_mock = MagicMock(return_value=draupnir_user_mock)
    ...
    generate_draupnir_config = MagicMock()
    monkeypatch.setattr(synapse, "generate_draupnir_config", generate_draupnir_config)
    ...
    harness.charm._draupnir.enable_draupnir(charm_state, admin_access_token)
    ...
    generate_draupnir_config.assert_called_once_with(
        container=ANY, access_token=draupnir_access_token, room_id=room_id
    )
...
def test_enable_draupnir_room_none(harness: Harness, monkeypatch: pytest.MonkeyPatch) -> None:
    """
    arrange: start the Synapse charm, set server_name, mock calls to validate args,
...
def test_enable_draupnir_container_off(harness: Harness, monkeypatch: pytest.MonkeyPatch) -> None:
    """
    arrange: start the Synapse charm, set server_name, mock container to not connect.
    act: call enable_draupnir.
    ...
    charm_state = harness.charm.build_charm_state()
    harness.charm._draupnir.enable_draupnir(charm_state, token_hex(16))

    register_user_mock.assert_not_called()


def test_enable_draupnir_admin_access_token(
    harness: Harness, monkeypatch: pytest.MonkeyPatch
) -> None:
    """
    ...
    monkeypatch.setattr(harness.charm._draupnir, "_token_service", token_service)

    assert harness.charm._draupnir._admin_access_token == token_mock

    monkeypatch.setattr(token_service, "get", MagicMock(return_value=None))
    monkeypatch.setattr(harness.charm._draupnir, "_token_service", token_service)

    assert harness.charm._draupnir._admin_access_token is None
...
def test_admin_access_token_no_connection(
    ...
    result = harness.charm._draupnir._admin_access_token
...
def test_admin_access_token_no_token(
    ...
    charm_state = harness.charm.build_charm_state()
    harness.charm._draupnir.enable_draupnir(charm_state, token_service_mock)
...
def test_on_collect_status_not_main_unit(
    ...
    act: call _on_collect_status.
    assert: no actions is taken because Draupnir is enabled only in main unit.
    """
...
def test_on_collect_status_not_main_unit_and_is_started(
    ...
    """
    arrange: start the Synapse charm, set server_name, unit has Draupnir started.
    act: call _on_collect_status.
    assert: Draupnir is stopped since this is not the main unit.
    """
    harness.add_relation(
        synapse.SYNAPSE_PEER_RELATION_NAME,
        "synapse",
    ...
    event_mock.add_status.assert_not_called()
    enable_draupnir_mock.assert_not_called()
    container_mock.stop.assert_called_with("draupnir")
