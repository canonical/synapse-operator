...
build-rock: .check-rockcraft $(ROCK_DIR_PATH)/$(ROCK_DYNAMIC_ARTIFACT) ## Build the ROCK OCI image.

$(ROCK_DIR_PATH)/$(ROCK_DYNAMIC_ARTIFACT):
	@$(call msg,"--> Building ROCK artifact: $(ROCK_DYNAMIC_ARTIFACT)...")
	@cd $(ROCK_DIR_PATH) && $(ROCKCRAFT_PACK_CMD)
	@mv $(ROCK_DIR_PATH)/$(ROCK_STATIC_ARTIFACT) $(ROCK_DIR_PATH)/$(ROCK_DYNAMIC_ARTIFACT)

publish-rock: .check-skopeo $(ROCK_DIR_PATH)/$(ROCK_DYNAMIC_ARTIFACT) $(REGISTRY_CHECK_TARGET) ## Push the ROCK OCI image to the registry, if not already present.
	...
	@{ \
		...
		$(SKOPEO_COPY_CMD) oci-archive:$(ROCK_DIR_PATH)/$(ROCK_DYNAMIC_ARTIFACT) docker://$(ROCK_IMAGE); \
	}

publish-rock-force: .check-skopeo $(ROCK_DIR_PATH)/$(ROCK_DYNAMIC_ARTIFACT) $(REGISTRY_CHECK_TARGET) ## Force push the ROCK OCI image to the registry.
